...
boxplot(hw2$waiting, horizontal=TRUE, col = "royalblue",
...
```{r q04_boxplot2, fig.height=2.5}
ggplot(hw2, aes(x = 1, y = waiting)) + 
...
ggplot(hw2, aes(x = waiting)) +
  ...
  stat_function(fun = dnorm, 
                args = list(mean = mean(hw2$waiting), sd = sd(hw2$waiting)),
                lwd = 1.5, col = "red") +
...
```{r q06_histogram_with_counts_and_Normal}
...
The Pearson correlation coefficient is `r round(cor(hw2$waiting, hw2$eruptions),3)`. This indicates a strong positive (or direct) and nearly linear relationship between eruption duration and waiting time.
...
hw2$timegroup <- 
  ifelse(hw2$eruptions < 3, "Less than 3 hours", "Greater than 3 hours")
...
by(hw2$waiting, hw2$timegroup, mosaic::favstats)
```
