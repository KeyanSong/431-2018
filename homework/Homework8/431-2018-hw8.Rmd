...
An appropriate response to this question will be at least 200 words long. Try to keep it below 500 words.
...
In questions 2-6, you will build and present a multiple regression model for plasma retinol levels. I would do so like this:
...
Then use the following code to select a training sample from the data set (to be used in questions 2-6) and a test sample (to be used in question 7). Feel free to use `431` as your seed - that is what we'll do in the Answer Sketch.
...
Now use the `hw8_training` data frame to build a new model for your outcome (as decided in Question 2) using the following 10 predictors: `age`, `sex`, `smoking`, `bmi`, `vitamin`, `calories`, `fat`, `fiber`, `alcohol`, `cholesterol`. Call that model `model_10`.

Compare `model_10` to `model_04` in terms of adjusted R^2^, and residual standard error. Which model performs better on these summaries, in this training sample?
...
a. Calculate the prediction errors in each case, then combine the results from the two models, following the code in section 14.1.
b. Visualize the prediction errors in each model, using the code in section 14.2.
c. Form the table comparing the model predictions, using the code in section 14.3. Compare the models in terms of MAPE, MSPE and maximum prediction error.

What conclusions do you draw about which model (`model_10` or `model_04`) is preferable? Is this the same conclusion you drew in Question 5?
